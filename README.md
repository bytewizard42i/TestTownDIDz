...
| **Data** — WHAT data, served at what tier | The librarian who checks your card | [HelixCTW](https://github.com/bytewizard42i/HelixCTW) | CockroachDB cluster + Bedrock agent + tiered gating live (hackathon build); kernel `DataGateway` adapter (`@didz/adapter-helixctw`) BUILT — tiered queries, one-bit disclosure, hash-verified cold retrieval, on a `helix_testtown` twin instance (frozen hackathon deployment untouched) |
